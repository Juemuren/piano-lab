...
- [楽譜エディター](#楽譜エディター): ABC Notation で楽譜を作成でき、演奏した音符の自動記録、レンダリング、再生、SVG/PNG/PDF/MIDI ファイルへのエクスポートに対応
- [仮想鍵盤](#仮想鍵盤): A0 から C8 までの完全な 88 鍵を提供し、マウス、タッチ画面、コンピューターキーボード、ゲームパッド、MIDI 入力デバイスでの演奏に対応
...
- 複数のプリセットを提供し、完全なカスタマイズも可能
- 任意の数のフィルターとイコライザーを使用でき、すべてのパラメーターを調整可能
...
- マウス、タッチ画面、コンピューターキーボード、ゲームパッド、MIDI 入力デバイスでの演奏に対応し、それぞれ個別に有効/無効を切り替え可能
- コンピューターキーボードは単列半音階のレイアウトを採用し、キーマッピングのカスタマイズも可能
- ゲームパッドは左右のトリガーで鍵盤を演奏し、スティックとボタンで音符を選択可能
...
- ゲームパッド入力は [Gamepad API](https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API) によって実現し、Xbox レイアウトと互換性のあるすべてのゲームパッドに対応
- MIDI デバイスの入力は [Web MIDI API](https://developer.mozilla.org/en-US/docs/Web/API/Web_MIDI_API) によって実現
