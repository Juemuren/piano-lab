...
- [音声シンセサイザー](#音声シンセサイザー): オシレーター、音量、倍音数、エンベロープ、スペクトル、伝達関数をまとめて調整
- [楽譜エディター](#楽譜エディター): ABC Notation で楽譜を書き、リアルタイムにレンダリングして自動演奏
- [仮想鍵盤](#仮想鍵盤): マウスまたはタッチ操作で演奏でき、楽譜再生の状態と同期してハイライト

多言語 UI、モバイル表示、ダークモードに対応し、[デスクトップアプリ][link-release]としてもインストールできます。
...
音声シンセサイザーは、エンベロープ、スペクトル、伝達関数の 3 つの折りたたみ可能なモジュールで構成されています。
...
エンベロープには ADSR を使用し、完全にカスタマイズできます。

- アタック時間、ディケイ時間、リリース時間、サステインゲイン、無音ゲインを変更可能
- 振幅エンベロープ曲線のプレビューにより、パラメーター変更が音のダイナミクスへ与える影響を確認可能

### スペクトル

スペクトルモジュールでは倍音振幅を完全にカスタマイズできます。また、複数のプリセットを提供し、対応する数式を直接レンダリングします。
...
伝達関数モジュールは、音の伝搬過程における振幅周波数特性と位相周波数特性をシミュレートします。伝達関数は周波数に対して連続的で完全なカスタマイズには向かないため、プリセットと調整可能なパラメーターを提供し、各倍音への影響をグラフでプレビューします。
...
振幅周波数特性と位相周波数特性をプレビューするときは、指定した基音周波数または音高を選択できます。

### 楽譜エディター

- テキストで楽譜を書き、リアルタイムにレンダリング
- 自動演奏に対応し、楽譜と鍵盤に視覚的なフィードバックを表示
- 楽譜上の音符をクリックするとその音を再生し、選択した音符から楽譜全体の演奏を開始可能
- テンポ、拍子、調号を変更でき、和音、反復、複数声部にも対応
- `.abc` ファイルのインポートとエクスポートに対応
- レンダリングした楽譜を SVG または PNG 画像としてエクスポート可能
- 「きらきら星」から「春日影」まで、複雑さの異なる複数のプリセット楽譜を収録

### 仮想鍵盤

- 音声シンセサイザーと同じ音色設定を共有
- 楽譜の自動演奏中、現在の音符が鍵盤のハイライト状態に同期して反映されます
...
- SVG は abcjs から直接提供され、PNG は Canvas API で SVG を変換して生成
