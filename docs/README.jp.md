...
フィルター、イコライザー、リバーブ、コンプレッション、パンニング、ウェーブシェーピングなど、多彩なエフェクトを含みます。
...
ここで $A$ は初期振幅、$\alpha$ は減衰係数、$D$ と $T$ はそれぞれ遅延時間と持続時間です。また、$r[n] \sim U[-1,1]$ は一様乱数であり、インパルス応答を正負均等に分布させることで、畳み込み時の大きな直流ゲイン $H(0)=\int_{-\infty}^{\infty}h(t)\mathrm{d}t$ を防ぎます。
