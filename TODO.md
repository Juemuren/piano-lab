...
- [ ] 取个更独特的应用名称和图标
- [x] 把脚注中关于乐谱编辑器的提示移动到播放控制条下
- [ ] 在脚注中补充关于应用的说明
- [ ] 当开启键盘控制时，在设置面板中补充关于键位的提示 `当前的键位映射已显示在琴键上。按 ctrl 可以临时降八度，按 z 可以永久降八度；按 shift 可以临时升八度，按 x 可以永久升八度`
...
- [x] 键盘操作增加组合键功能：按 `ctrl` 可以临时降八度，按 `shift` 可以临时升八度
...
- [x] 显示已连接的 MIDI 输入设备名称和连接状态，并在 MIDI 权限失败 / 浏览器不支持时给出提示
- [x] 可以选择单个 MIDI 输入设备，而不是监听全部输入
