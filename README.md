...
- 晚期尾音使用正态随机数加权的指数衰减采样，支持修改延迟时间、持续时间、振幅系数和衰减系数
- 可绘制脉冲响应的公式和图像
- 支持按需启用或禁用混响效果
...
- 电脑键盘默认使用如下的按键映射方案，并可以用 `Z` / `X` 切换八度，以及 `Ctrl` / `Shift` 组合键临时改变八度
...
**晚期尾音**模拟声音在空间中经多次反射后叠加而成的密集回声，使用指数衰减的包络作为脉冲响应。其表达式为

$$
h_l[n]=A\mathcal{N}(0,1)e^{-\alpha(n-Df_s)}
$$

其中 $A$ 为初始振幅、$\alpha$ 为衰减系数，$D$ 和 $T$ 分别为延迟时间和持续时间。而 $\mathcal{N}(0,1)$ 是均值为 $0$，标准差为 $1$ 的正态随机数，目的是让脉冲响应正负分布均匀，防止卷积时产生很大的直流增益 $H(0)=\int_{-\infty}^{\infty}h(t)\mathrm{d}t$ 。

混响效果提供浴室/车库/大厅/教堂四种预设，以模拟从小到大的各种空间。
...
声像效果使用 Web Audio API 的 [PannerNode](https://developer.mozilla.org/en-US/docs/Web/API/PannerNode) 实现，用于在立体声场或三维空间中定位音源。

PannerNode 支持等功率平移算法和头部相关传递函数两种声像模型。HRTF 通过模拟人耳对来自不同方向声音的响应来提供更真实的空间感。其关键参数为

- **坐标**和**方位角**，控制音源在空间中的位置和朝向
- **距离**，控制音源与听者的距离，支持线性、反比和指三种距离模型
...
调制通过一个低频振荡器周期性地改变被调制量来实现，根据被调制量的不同，可以产生颤音、合唱等各种效果。
