...
- [虚拟琴键](#虚拟琴键)：提供从 A0 到 C8 的完整 88 键，支持使用鼠标、触屏、电脑键盘、手柄和 MIDI 输入设备进行演奏
...
- 支持使用鼠标、触屏、电脑键盘、手柄和 MIDI 输入设备进行演奏，并可分别启用或禁用
- 电脑键盘使用单排半音阶的布局方案，且支持自定义键位映射
- 手柄使用左右扳机演奏琴键，可通过摇杆和按键选择音符
- MIDI 输入设备能够显示连接状态，并可在设备列表中选择一个进行监听
...
- 手柄的输入通过 [Gamepad API](https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API) 实现，支持所有兼容 Xbox 布局的手柄
- MIDI 设备的输入通过 [Web MIDI API](https://developer.mozilla.org/en-US/docs/Web/API/Web_MIDI_API) 实现
